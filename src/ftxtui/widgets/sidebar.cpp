...
Element build_sidebar(const SidebarModel& s) {
    ...
    // 标题
    std::string title = s.title.empty() ? "新会话" : s.title;
    rows.push_back(ftxui::text(title) | ftxui::bold);
    rows.push_back(sidebar_rule());

    // 项目 / 分支 / Agent（项目与 Agent 文本用米白色）
    rows.push_back(kv("项目", s.project.empty() ? "—" : s.project, kInfoText));
    if (!s.branch.empty())
        rows.push_back(kv("分支", s.branch));
    if (!s.agent.empty())
        rows.push_back(kv("Agent", s.agent, kInfoText));

    rows.push_back(sidebar_rule());

    // 上下文占用
    rows.push_back(ftxui::text("上下文"));
    rows.push_back(ftxui::hbox({
        build_context_gauge(s.context_used, s.context_limit),
    }));
    std::string ctx_str = std::to_string(s.context_used) + " / " +
    ...
    // 成本
    {
        ...
        rows.push_back(kv("成本", buf, kInfoText));
    }

    // 模型
    rows.push_back(kv("模型", s.model.empty() ? "—" : s.model, kInfoText));
