...
// Save the old terminal configuration and restore it on exit.
  ...
  // 鼠标事件支持：启用 ENABLE_MOUSE_INPUT 并禁用 ENABLE_QUICK_EDIT_MODE，
  // 否则控制台不会上报 MOUSE_EVENT（快速编辑模式会拦截鼠标用于原生选中）。
  // 修改 QUICK_EDIT 需同时设置 ENABLE_EXTENDED_FLAGS。
  const int enable_mouse_input = 0x0010;
  const int enable_extended_flags = 0x0080;
  const int disable_quick_edit_mode = 0x0040;
  // 禁用 ENABLE_PROCESSED_INPUT：否则 Ctrl+C 被系统转为 CTRL_C_EVENT 信号
  // （→ SIGINT → 立即退出），永远到不了应用层的 CatchEvent 双击退出逻辑。
  // 清除后 Ctrl+C 以 KEY_EVENT（UnicodeChar=0x03）进入输入流，由上层处理。
  in_mode &= ~enable_processed_input;
  in_mode &= ~enable_echo_input;
  in_mode &= ~enable_line_input;
  in_mode &= ~disable_quick_edit_mode;
  ...
  for (const auto& r : records) {
    switch (r.EventType) {
      case KEY_EVENT: {
        ...
        // Ctrl+←/→：Windows 控制台把 Ctrl+箭头 报为 VK_LEFT/VK_RIGHT + Ctrl 键状态，
        // 改写为 kitty 键盘协议序列 \x1b[1;5D / \x1b[1;5C，上层按 Special 事件识别。
        if ((key_event.wVirtualKeyCode == VK_LEFT ||
             key_event.wVirtualKeyCode == VK_RIGHT) &&
            (key_event.dwControlKeyState &
             (LEFT_CTRL_PRESSED | RIGHT_CTRL_PRESSED)) != 0) {
          const std::string seq =
              key_event.wVirtualKeyCode == VK_LEFT ? "\x1b[1;5D" : "\x1b[1;5C";
          for (char c : seq) {
            terminal_input_parser.Add(c);
          }
          continue;
      ...
      case WINDOW_BUFFER_SIZE_EVENT:
        ...
        break;
      case MOUSE_EVENT: {
        // Windows 控制台鼠标事件 → FTXUI Event::Mouse。
        // 上游 v7.0.3 忽略 MOUSE_EVENT（"TODO: Implement later"），
        // 导致 Windows 下点击/滚轮/文本选中全部失效，此处补全。
        const auto& me = r.Event.MouseEvent;
        Mouse mouse;
        const DWORD buttons = me.dwButtonState;
        const DWORD flags = me.dwEventFlags;

        if (flags & MOUSE_WHEELED) {
          const short delta = static_cast<short>(HIWORD(buttons));
          mouse.button = delta > 0 ? Mouse::WheelUp : Mouse::WheelDown;
          ...
          const DWORD pressed = FROM_LEFT_1ST_BUTTON_PRESSED |
                                RIGHTMOST_BUTTON_PRESSED |
                                FROM_LEFT_2ND_BUTTON_PRESSED |
                                FROM_LEFT_3RD_BUTTON_PRESSED |
                                FROM_LEFT_4TH_BUTTON_PRESSED;
          mouse.motion = (buttons & pressed) ? Mouse::Pressed : Mouse::Released;
        }

        mouse.shift =
            (me.dwControlKeyState & SHIFT_PRESSED) != 0;
        mouse.control = (me.dwControlKeyState &
                         (LEFT_CTRL_PRESSED | RIGHT_CTRL_PRESSED)) != 0;
        mouse.meta = (me.dwControlKeyState &
                      (LEFT_ALT_PRESSED | RIGHT_ALT_PRESSED)) != 0;

        // 1-based 屏幕坐标；HandleTask 中会减去帧原点（cursor_x_/y_）。
        // 全屏备用屏下 cursor_ 恒为 1，1-based 坐标换算后即 0-based 帧坐标。
        mouse.x = me.dwMousePosition.X + 1;
        mouse.y = me.dwMousePosition.Y + 1;

        event_buffer.Push(Event::Mouse("", mouse));
      } break;
      case MENU_EVENT:
      case FOCUS_EVENT:
        break;
