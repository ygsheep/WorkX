...
// ---------- 跨平台适配（WorkX 为 MSVC/Win + Linux/macOS 三平台补丁；保持 MIT 不增限） ----------
// 1) NOMINMAX：阻止 <windows.h>（通过 libcurl/WinSock 间接 include）
//    污染出 `min/max` 函数式宏，宏展开会把成员名 `index_ch_in_html_->min(...)`
//    等或后续 STL 调用炸成非法语法。
#if !defined(NOMINMAX)
#  define NOMINMAX
#endif
...
// BSD 派生的位宽整型（u_int16_t / u_int64_t）在 MSVC 标准库默认不提供。
// 这里用 iso 固定宽无符号整数同型 typedef 补足；放进全局命名空间以便库内部直接使用。
#if defined(_MSC_VER) && !defined(u_int16_t_DEFINED)
typedef std::uint16_t u_int16_t;
typedef std::uint32_t u_int32_t;
typedef std::uint64_t u_int64_t;
#  define u_int16_t_DEFINED 1
#endif
...
class Converter {
 ...
 private:
  ...
  static void LTrim(std::string *s) {
    auto is_not_ws = [](unsigned char c) noexcept { return !std::isspace(c); };
    (*s).erase(
        (*s).begin(),
        std::find_if((*s).begin(), (*s).end(), is_not_ws));
  }

  // Trim from end (in place)
  Converter * RTrim(std::string *s, bool trim_only_blank = false) {
#if __cplusplus >= 201703L && defined(__cpp_lib_string_contains)
    // std::isblank C++11；老工具链无则退回 isspace
#endif
    auto is_not_ws  = [](unsigned char c) noexcept { return !std::isspace(c); };
    auto is_not_blank = [](unsigned char c) noexcept {
      // 避免 <cctype> isblank 存在 MSVC 19.X 偶发性符号缺失
      return !(c == ' ' || c == '\t');
    };
    (*s).erase(
        std::find_if(
            (*s).rbegin(),
            (*s).rend(),
            trim_only_blank ? is_not_blank : is_not_ws
        ).base(),
        (*s).end());
  ...
  std::string ExtractAttributeFromTagLeftOf(std::string attr) {
    ...
    // NOTE: index_ch_in_html_ 不统计 '=' 和 '"'（ParseCharInTag 对它们直接 return true），
    // 用它做长度会截短 tag，较长 URL 取不到闭合引号导致 href 丢失；改为向后找 '>'。
    auto gt = html_.find('>', offset_lt_);
    if (gt == std::string::npos) return "";
    auto tag = html_.substr(offset_lt_, gt - offset_lt_);
