...
#
# ============================================================================
# WorkX 本地补丁副本（基于 vcpkg Catch2 的 CatchAddTests.cmake）
#
# 为什么需要这份副本：
#   Windows 下 MSBuild 的 post-build 测试发现步骤运行在 GBK(936) 控制台代码页，
#   CMake 的 execute_process 捕获子进程 stdout 时会按 GBK 转码，导致测试名中的
#   中文（UTF-8）被破坏成 mojibake，`string(JSON ...)` 解析失败、构建报错
#   "Missing ',' or '}' in object declaration"。
#
# 修复：在捕获测试列表前先执行 `chcp 65001` 把控制台切到 UTF-8 代码页。
#   （chcp 必须以独立 execute_process 直接子进程运行才生效；包进 .bat 包装脚本
#    或子进程内 SetConsoleOutputCP 均无效，因为 execute_process 的子进程无控制台。）
#
# 若上游 Catch2 更新了 CatchAddTests.cmake，请同步本文件（仅多了上面一行 chcp）。
# ============================================================================
...
function(catch_discover_tests_impl)
  ...
  # [WorkX patch] Windows GBK 控制台下 execute_process 会转码 UTF-8 输出，
  # 先切到 UTF-8 代码页再捕获测试列表，避免中文测试名 JSON 解析失败。
  if(WIN32)
    execute_process(COMMAND chcp 65001 OUTPUT_QUIET ERROR_QUIET)
  endif()
